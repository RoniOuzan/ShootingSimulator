...
public class TrajectoryChooser {
    ...
    // TODO: make it work with radial velocity
    public Trajectory findBestTrajectory() {
        ...
        return this.builder.simulateTrajectory(
                binarySearchBestVelocityForAngle(bestAngle, (physicalValues.minVel + physicalValues.maxVel) / 2.0).getInitialShootingVelocity().getNorm(),
    ...
    private double getCostAtAngle(double angle) {
        ...
        double vReq = binarySearchBestVelocityForAngle(angle, (physicalValues.minVel + physicalValues.maxVel) / 2.0)
                .getInitialShootingVelocity().getNorm();

        // Simulate the trajectory
        Trajectory trajectory = this.builder.simulateTrajectory(vReq, Rotation2d.fromDegrees(angle));

        // Check if it actually hits (if it hits the target, return cost, else return Infinity)
        if (!this.builder.isInsideTarget(trajectory.getFinalSample())) return Double.MAX_VALUE;

        return calculateTrajectoryCost(trajectory);
    }
