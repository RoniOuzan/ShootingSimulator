...
const PRESETS: Record<Exclude<CostPreset, "CUSTOM">, Omit<SharedConfig["cost"], "preset">> = {
  ...
  SLOW_SHOT: { 
    robustnessWeight: 0.5, initialVelocityWeight: 1.0, impactVelocityWeight: 0.3, timeOfFlightWeight: 0.0, entryAngleWeight: 0.0, targetImpactAngle: 0 
  },
  FAST_ARRIVAL: { 
      ...
      {!isCollapsed && (
        <div className="sidebar-content">
          ...
          <div className="config-card target">
            <h3 className="card-title"><span className="icon">🎯</span> Target Bounds</h3>
            <ControlSlider label="Target Height (Y)" value={target.targetY} min={0.5} max={5} step={0.05} unit="m" onChange={(v) => updateConfig("target", { targetY: v })} />
          ...
          <div className="config-card aerodynamics">
            <h3 className="card-title"><span className="icon">💨</span> Aerodynamics</h3>
            <div className="card-row">
          ...
          <div className="config-card hardware">
            <h3 className="card-title"><span className="icon">⚙️</span> Hardware Limits</h3>
            <div className="card-row">
          ...
          <div className="config-card cost">
            <h3 className="card-title"><span className="icon">⚖️</span> Evaluation Cost Function</h3>
            
            <div className="preset-grid">
              <button className={`preset-btn ${cost.preset === 'ROBUST' ? 'active' : ''}`} onClick={() => handlePresetChange('ROBUST')}>🛡️ Most Robust</button>
              ...
              <button className={`preset-btn ${cost.preset === 'SLOW_SHOT' ? 'active' : ''}`} onClick={() => handlePresetChange('SLOW_SHOT')}>🐢 Slow Shot</button>
              <button className={`preset-btn ${cost.preset === 'BALANCED' ? 'active' : ''}`} onClick={() => handlePresetChange('BALANCED')}>⚖️ Balanced</button>
              <button className={`preset-btn ${cost.preset === 'CUSTOM' ? 'active' : ''}`} onClick={() => handlePresetChange('CUSTOM')}>⚙️ Custom Weights</button>
            </div>

            <div className="weights-status-header">
              {isCustom ? (
                <span className="status-custom">Active Custom Multipliers</span>
              ) : (
                <span className="status-locked">🔒 Preset Multipliers (Read-Only)</span>
              )}
            </div>

            <div className={`weights-container ${!isCustom ? 'locked' : ''}`}>
              {/* Using the new compact WeightControl for multipliers */}
              ...
              {/* Keep this as a ControlSlider because it is a physical physical target, not a weight */}
