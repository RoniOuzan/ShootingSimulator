...
type DragState = {
  type:
    | "OBSTACLE_CIRCLE"
    | "OBSTACLE_CIRCLE_RESIZE"
    | "OBSTACLE_POLYGON"
    | "OBSTACLE_POLYGON_VERTEX"
    | "PANNING";
  id?: string;
  startX: number;
  startY: number;
...
export default function ObstacleManagingModel({
  config,
  updateConfig,
  onClose,
}: Props) {
  ...
  const targetX = config.target.center.x;
  const targetY = config.target.center.y;
  ...
  const handleRemoveObstacle = (id: string) => {
    updateConfig(
      "obstacles",
      obstacles.filter((obs) => obs.id !== id),
    );
  ...
  const handlePointerDownObstacle = (
  ...
  const handlePointerDownCircleResize = (
    e: React.PointerEvent,
    obs: ObstacleConfig,
  ) => {
    e.stopPropagation();
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (
      dragState.type === "PANNING" &&
      dragState.initialPanX !== undefined &&
      dragState.initialPanY !== undefined
    ) {
    ...
    if (dragState.initialObstacles) {
      const updatedObstacles = dragState.initialObstacles.map((obs) => {
        ...
        if (obs.type === "CIRCLE" && dragState.type === "OBSTACLE_CIRCLE") {
          return {
            ...obs,
            center: { x: obs.center.x + deltaX_m, y: obs.center.y + deltaY_m },
          };
        }
        if (
          obs.type === "CIRCLE" &&
          dragState.type === "OBSTACLE_CIRCLE_RESIZE"
        ) {
          const initialObs = dragState.initialObstacles!.find(
            (o) => o.id === dragState.id,
          );
          if (initialObs && initialObs.type === "CIRCLE") {
            const currentHandleX =
              initialObs.center.x + initialObs.radius + deltaX_m;
            const currentHandleY = initialObs.center.y + deltaY_m;
            const newRadius = Math.hypot(
              currentHandleX - initialObs.center.x,
              currentHandleY - initialObs.center.y,
            );
        ...
        if (
          obs.type === "POLYGON" &&
          dragState.type === "OBSTACLE_POLYGON_VERTEX" &&
  ...
  const handleUpdateObstacleData = (
    id: string,
      ...
      <div className="env-editor-modal">
        <div className="env-editor-header">
          ...
          <div className="header-controls">
            ...
            <button
              className="add-btn poly"
              onClick={() => handleAddObstacle("POLYGON")}
            >
              + Polygon
            </button>
        ...
        <div className="env-editor-body">
          ...
          <div className="env-editor-canvas-container">
                    ...
                    <g key={obs.id}>
                      <circle
                        ...
                        style={{
                          cursor:
                            dragState?.id === obs.id &&
                            dragState.type === "OBSTACLE_CIRCLE"
                              ? "grabbing"
                              : "grab",
                        }}
                      ...
                      <circle
                        ...
                        onPointerDown={(e) =>
                          handlePointerDownCircleResize(e, obs)
                        }
                      />
                    </g>
                  );
                    ...
                    <g key={obs.id}>
                      <polygon
                        ...
                        style={{
                          cursor:
                            dragState?.id === obs.id &&
                            dragState.type === "OBSTACLE_POLYGON"
                              ? "grabbing"
                              : "grab",
                        }}
                        onPointerDown={(e) => handlePointerDownObstacle(e, obs)}
                        ...
                        <circle
                          ...
                          style={{
                            cursor:
                              dragState?.type === "OBSTACLE_POLYGON_VERTEX" &&
                              dragState.id === obs.id &&
                              dragState.vertexIndex === i
                                ? "grabbing"
                                : "crosshair",
                          }}
                          onPointerDown={(e) =>
                            handlePointerDownVertex(e, obs, i)
                          }
                        />
                      ))}
                    </g>
            ...
            <div className="env-editor-footer">
              <p>
                ℹ️ <b>Drag background</b> to pan. <b>Drag shapes</b> to move
                them. Select an obstacle and press <b>Backspace</b> to delete.
              </p>
            </div>
          </div>
          ...
          <div className="env-editor-properties">
            <h3>Properties</h3>
            {obstacles.length === 0 && (
              <p className="no-props">
                No obstacles. Add one from the top bar.
              </p>
            )}
                  ...
                  <div className="prop-header">
                    <div className="prop-title">
                      <input
                        ...
                        value={obs.name}
                        onChange={(e) =>
                          handleUpdateObstacleData(obs.id, {
                            name: e.target.value,
                          })
                        }
                      />
                      ...
                      <div className="number-row">
                        <div className="num-box">
                          ...
                          <input
                            type="number"
                            step="0.1"
                            value={Number(obs.center.x.toFixed(2))}
                            onChange={(e) =>
                              handleUpdateObstacleData(obs.id, {
                                center: {
                                  ...obs.center,
                        ...
                        <div className="num-box">
                          ...
                          <input
                            type="number"
                            step="0.1"
                            value={Number(obs.center.y.toFixed(2))}
                            onChange={(e) =>
                              handleUpdateObstacleData(obs.id, {
                                center: {
                                  ...obs.center,
                      ...
                      <div className="number-row full">
                        <div className="num-box full">
                          ...
                          <input
                            ...
                            step="0.1"
                            min="0.1"
                            value={Number(obs.radius.toFixed(2))}
                            onChange={(e) =>
                              handleUpdateObstacleData(obs.id, {
                                radius: Math.max(
                                  0.1,
