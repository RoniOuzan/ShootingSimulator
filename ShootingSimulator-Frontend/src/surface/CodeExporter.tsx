...
function buildGeneratedCode(models: any, sharedConfig: SharedConfig, totalPoints: number): string {
  ...
  // Extract configuration subgroups for cleaner template literals
  const aero = sharedConfig.aerodynamics;
  const hw = sharedConfig.hardware;
  const cost = sharedConfig.cost;
  const origin = sharedConfig.origin;
  const target = sharedConfig.target;

  // Conditionally build the custom cost details block
  const customCostDetails = cost.preset === "CUSTOM" 
    ? `\n//   * Robustness Wt:        ${cost.robustnessWeight}` +
      `\n//   * Init Vel Wt:          ${cost.initialVelocityWeight}` +
      `\n//   * Impact Vel Wt:        ${cost.impactVelocityWeight}` +
      `\n//   * Flight Time Wt:       ${cost.timeOfFlightWeight}` +
      `\n//   * Entry Angle Wt:       ${cost.entryAngleWeight}` +
      `\n//   * Target Impact Angle:  ${cost.targetImpactAngle}°`
    : "";
...
//
// Environment & Target:
// - Initial Height:         ${origin.initialY} m
// - Target Height:          ${target.center.y} m
// - Target Radius:          ${target.radius} m
//
// Aerodynamics:
// - Projectile Shape:       ${aero.shape}
// - Mass (kg):              ${aero.mass}
// - Radius (m):             ${aero.radius}
// - Drag Coefficient:       ${aero.dragCoeff}
//
// Hardware Limits:
// - Flywheel Velocity:      ${hw.minVel} to ${hw.maxVel} m/s
// - Pivot Angle:            ${hw.minAngle}° to ${hw.maxAngle}°
// - Velocity Error Margin:  ±${hw.velocityError} m/s
// - Angle Error Margin:     ±${hw.angleError}°
//
// Optimizer / Cost Config:
// - Active Preset:          ${cost.preset}${customCostDetails}
// ==========================================================
